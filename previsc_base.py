...
<svg class="pv-ondas" viewBox="0 0 1600 900" preserveAspectRatio="none"
  ...
  <g fill="none" stroke-linecap="round">
    <path d="M1180,-40 C1520,120 1660,430 1610,900" stroke="#00E5FF" stroke-width="6" opacity=".85"/>
    <path d="M1240,-40 C1580,140 1720,450 1670,900" stroke="#3ED18A" stroke-width="5" opacity=".7"/>
    <path d="M1300,-40 C1640,160 1780,470 1730,900" stroke="#B14BFF" stroke-width="4" opacity=".6"/>
    <path d="M-260,940 C-40,600 220,430 560,470" stroke="#00E5FF" stroke-width="6" opacity=".8"/>
    <path d="M-300,1000 C-60,640 220,470 600,520" stroke="#3ED18A" stroke-width="5" opacity=".6"/>
  </g>
</svg>
"""
...
/* Botões Padrões (Formato Pílula + Gradiente Exato do Print) */
.stButton > button, .stDownloadButton > button, .stFormSubmitButton > button {{
  background: linear-gradient(90deg, {VERDE} 0%, {AZUL} 50%, {ROXO} 100%) !important;
  color: #fff !important; 
  border: none !important; 
  border-radius: 20px !important; /* Arredondado estilo pílula */
...
/* ---------- SIDEBAR: Cartão Branco Flutuante (Restaurado conforme o Print) ---------- */
section[data-testid="stSidebar"] {{
  background: transparent; padding: 1.1rem .8rem 1.1rem 1.1rem;
}}
section[data-testid="stSidebar"] > div:first-child {{
  background: #f8f9fa; /* Cartão branco/claro */
  border-radius: 22px;
  box-shadow: 0 26px 60px rgba(0,0,0,.35); overflow: hidden;
}}
/* Botões da barra lateral mantêm o gradiente e formato pílula */
section[data-testid="stSidebar"] .stButton > button {{
  width: 100%; margin-bottom: 0.6rem; padding: 0.8rem 0.5rem !important; font-size: 0.95rem;
}}
/* Texto do rodapé da barra lateral em cinza */
section[data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p {{
  color: #6d6d7a !important; font-size: .8rem; line-height: 1.45;
}}
section[data-testid="stSidebar"] [data-testid="stVerticalBlock"] {{ gap: .2rem; }}

.pv-logo {{ text-align: center; padding: 1.4rem 0 1.1rem; }}
/* FONTE COM COR MESCLADA (Gradiente Azul/Ciano) IGUAL AO PRINT */
.pv-logo .pv-logo-nome {{
  font-style: italic; font-weight: 900; font-size: 2.6rem; letter-spacing: -1px;
  background: linear-gradient(90deg, {AZUL} 0%, {CIANO} 100%);
  -webkit-background-clip: text; background-clip: text; color: transparent; line-height: 1;
}}
/* Subtítulo logo Verde igual ao print */
.pv-logo .pv-logo-sub {{
  font-style: italic; font-weight: 800; font-size: .65rem; letter-spacing: .6px;
...
.st-key-pv_cta_box .stButton > button {{ padding: .5rem 1.6rem !important; font-size: .95rem; border-radius: 10px !important; }}
...
def titulo_pagina(texto: str, descricao: str = "") -> None:
    ...
    if descricao:
        st.markdown(f"<p style='max-width:52ch;font-size:.9rem;color:#ffffff;'>{descricao}</p>", unsafe_allow_html=True)
...
def rodape_sidebar(texto: str = "Sistema interno desenvolvido pela equipe de Arrecadação para processar cálculos previdenciários (individuais e em lote) de Participantes Ativos e Autopatrocinados, integrado à consulta rápida das regras vigentes.") -> None:
    st.markdown(
        f"<div style='padding:1.6rem 1rem 1.4rem;'><p style='color:#6d6d7a !important;font-size:.78rem;line-height:1.45;margin:0;'>{texto}</p></div>",
        unsafe_allow_html=True,
    )
