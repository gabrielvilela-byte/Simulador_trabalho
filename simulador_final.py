...
def simular_cobranca_autopatrocinio(plano_nome, salario, aliq_escolhida=None, univali_migrante="Migrante", univali_tipo="Normal", idade_ou_tempo=30, faixa_opcao="Faixa 1", categoria_participante="Migrante"):
    ...
    # Default Fallback (FIESCPREV, SENACPREV, etc.) revertido para a lógica original onde não tem contribuição patronal na composição do boleto
        ...
        if st.button("Gerar cálculo", type="primary"):
            if salario_input > 0:
                ...
                if plano_selecionado == "UNIVALIPrevidencia":
                    ...
                    total_cobranca = arredondar(contrib_pura + valor_adm_base + valor_risco)
                    ...
                    if tx_adm_plano > 0:
                        col_b2.metric(f"Taxa Administração ({formatar_br(tx_adm_plano * 100)}%)", f"R$ {formatar_br(valor_adm_base)}")
                    else:
